...
def get_statistics():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Positive: {stats.get('positive', 0)} ({stats.get('positive_percentage', 0)}%)")
            print(f"Negative: {stats.get('negative', 0)} ({stats.get('negative_percentage', 0)}%)")
            print(f"Neutral: {stats.get('neutral', 0)} ({stats.get('neutral_percentage', 0)}%)")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, item in enumerate(history[-5:], 1):  # Show last 5
                print(f"\n{i}. {item.get('text', '')[:50]}...")
                print(f"   Sentiment: {item.get('sentiment')} ({item.get('confidence')}%)")
