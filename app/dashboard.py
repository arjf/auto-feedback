...
def analyze_text_api(text, model_type="textblob"):
    ...
    try:
        ...
        return {"error": "Cannot connect to API. Make sure Flask server is running on port 5000."}
...
with tab1:
    ...
    with col1:
        ...
        input_method = st.radio(
            "Input Method",
            ["Type/Paste Text", "Sample Feedback"],
            horizontal=True
        )
...
with tab2:
    ...
    if st.session_state.analysis_results:
        ...
        confidences = [r.get('confidence', 0) for r in st.session_state.analysis_results]
        ...
        with conf_col2:
            st.metric("Min Confidence", f"{min(confidences):.2f}%" if confidences else "N/A")
        with conf_col3:
            st.metric("Max Confidence", f"{max(confidences):.2f}%" if confidences else "N/A")
    else:
...
with tab3:
    ...
    if st.session_state.analysis_results:
        ...
        fig_timeline = px.scatter(
            ...
            size='confidence',
            hover_data=['text'],
            labels={'analysis_number': 'Analysis Number', 'confidence': 'Confidence (%)'}
        )
...
with tab4:
    ...
    if st.session_state.analysis_results:
        ...
        st.subheader(f"Recent Analyses ({len(st.session_state.analysis_results)} total)")
