...
def analyze():
    ...
    try:
        ...
        logger.info(f"Analysis complete: {result.get('sentiment')} ({result.get('confidence')}%)")
...
def stats():
    ...
    try:
        ...
        positive = sum(1 for item in analysis_history if item.get('sentiment') == 'Positive')
        negative = sum(1 for item in analysis_history if item.get('sentiment') == 'Negative')
        neutral = sum(1 for item in analysis_history if item.get('sentiment') == 'Neutral')
        
        avg_confidence = sum(item.get('confidence', 0) for item in analysis_history) / total
